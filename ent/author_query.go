...
func (aq *AuthorQuery) loadBooks(ctx context.Context, query *BookQuery, nodes []*Author, init func(*Author), assign func(*Author, *Book)) error {
	...
	query.Where(predicate.Book(func(s *sql.Selector) {
		s.Where(sql.InValues(author.BooksColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "author_books" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
